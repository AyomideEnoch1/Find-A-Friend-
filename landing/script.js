...
const init3DBackground = () => {
  ...
  const animate = () => {
    ...
    particlesMesh.rotation.x +=
      (mouseY * 0.5 - particlesMesh.rotation.x) * 0.05;
    particlesMesh.rotation.y +=
      (mouseX * 0.5 - particlesMesh.rotation.y) * 0.05;
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (mouse.x !== null && mouse.y !== null) {
        ...
        if (dist < config.mouseRadius) {
          const alpha =
            (1 - dist / config.mouseRadius) * (config.lineAlphaScale + 0.1);
          ctx.beginPath();
...
const initCustomCursor = () => {
  ...
  const interactiveElements = document.querySelectorAll(
    "button, a, .nav-link, .exp-card, .cap-card",
  );
...
const initScrollAnimations = () => {
  ...
  const sections = document.querySelectorAll(
    ".experience, .capabilities, .neural, .launch",
  );
...
const initButtons = () => {
  ...
  const handleButtonClick = (btn, action) => {
    ...
    btn.addEventListener("click", () => {
      ...
      if (action === "signup") {
        btn.innerHTML =
          '<span>Connecting...</span><i class="fas fa-spinner fa-spin"></i>';
        setTimeout(() => {
          btn.innerHTML =
            '<span>Join FAF</span><i class="fas fa-arrow-right"></i>';
          showLaunchpadModal();
        }, 1000);
        ...
        if (email && email.includes("@")) {
          btn.innerHTML =
            '<span>Requesting...</span><i class="fas fa-spinner fa-spin"></i>';
          setTimeout(() => {
            btn.innerHTML =
              '<span>Request Access</span><i class="fas fa-arrow-right"></i>';
            emailInput.value = "";
        ...
        <div class="modal-content" style="max-width: 550px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem; text-align: left;">
                ...
                <div style="background: var(--bg-tertiary); border: 1px solid var(--glass-border); padding: 1.2rem; border-radius: 16px; display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="https://expo.dev/artifacts/eas/9isrrqaJMf_fBHHM-YolNA7W3-EvO7g2tA3ZoGOSXiI.apk" download class="btn-premium" style="text-decoration: none; padding: 0.6rem 1.2rem; font-size: 0.8rem; background: var(--gradient-2);">
