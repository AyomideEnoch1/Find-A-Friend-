...
import { getListings, toggleSaveDeal, getMySavedDealIds, type VendorDeal } from '../lib/vendors'
...
export default function DealsScreen() {
  ...
  const categories = ['All', ...Array.from(new Set(
    deals.map(d => d.vendors?.category ?? 'Other').filter(Boolean)
  )).sort()]

  const filtered = deals.filter(d => {
    const cat = d.vendors?.category ?? 'Other'
    const matchCat = activeCategory === 'All' || cat === activeCategory
    const matchSearch = (d.vendors?.name ?? '').toLowerCase().includes(search.toLowerCase()) ||
    ...
    <SafeAreaView style={[s.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={[s.header, { borderBottomColor: theme.border }]}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          ...
          <Text style={[s.subtitle, { color: theme.textFaint }]}>Student-only discounts near you</Text>
        </View>
        <View style={{ width: 36 }} />
      </View>
          ...
          {categories.map((cat) => (
            <TouchableOpacity
              ...
              style={[
                ...
                activeCategory === cat && { backgroundColor: theme.accentBg, borderColor: theme.accentBorder },
              ]}
              onPress={() => setActiveCategory(cat)}>
              <Text style={[
                s.chipText, { color: theme.textMuted },
                activeCategory === cat && { color: theme.accent, fontFamily: typography.fontMedium },
              ]}>
                {cat}
              </Text>
            </TouchableOpacity>
      ...
      {loading ? (
        <View style={s.center}>
          ...
          <Text style={[s.loadingText, { color: theme.textMuted }]}>Loading deals...</Text>
        </View>
      ) : (
        <ScrollView
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((deal) => {
              ...
              return (
                <View
                  key={deal.id}
                  style={[s.dealCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                  <View style={s.dealTop}>
                    ...
                    <View style={[s.discountBadge, {
                      ...
                      <Text style={[s.discountText, { color }]}>{deal.discount}</Text>
                    </View>
                  </View>

                  {/* Footer */}
                  <View style={[s.dealFooter, { borderTopColor: theme.border2 }]}>
                    <View style={s.redeemWrap}>
                      <Text style={[s.redeemLabel, { color: theme.textFaint }]}>How to redeem:</Text>
                      <Text style={[s.redeemText, { color: theme.textMuted }]} numberOfLines={1}>
                        {deal.how_to_redeem}
                      </Text>
                    </View>
                    <TouchableOpacity
                      style={[
                        ...
                        isSaved && { backgroundColor: color + '20', borderColor: color },
                      ]}
                      ...
                      {savingId === deal.id ? (
                        ...
                        <Text style={[s.saveText, { color: isSaved ? color : theme.textMuted }]}>
                          {isSaved ? '✓ Saved' : '🔖 Save'}
                        </Text>
                      )}
                    </TouchableOpacity>
                  ...
                  {deal.valid_until && (
                    <Text style={[s.expiry, { color: theme.textFaint }]}>
                      Expires {new Date(deal.valid_until).toLocaleDateString('en-GB', {
...
const s = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: 8, paddingBottom: 12, borderBottomWidth: 0.5,
  },
  backBtn: {
  ...
  emptyText: { fontSize: 13, textAlign: 'center', fontFamily: typography.fontRegular, lineHeight: 20 },
  dealCard: {
    marginBottom: 10, borderRadius: 16, padding: 14, borderWidth: 0.5,
  },
  dealTop: { flexDirection: 'row', gap: 12, marginBottom: 12, alignItems: 'flex-start' },
  dealIcon: {
  ...
  dealName: { fontSize: 14, fontFamily: typography.fontSemiBold, marginBottom: 2 },
  dealLocation: { fontSize: 11, fontFamily: typography.fontRegular, marginBottom: 3 },
  dealTitle: { fontSize: 13, fontFamily: typography.fontMedium, lineHeight: 18 },
  discountBadge: {
    borderRadius: 10, paddingHorizontal: 8, paddingVertical: 4,
    borderWidth: 0.5, alignSelf: 'flex-start', flexShrink: 0,
  },
  ...
  redeemLabel: { fontSize: 10, fontFamily: typography.fontRegular, marginBottom: 2 },
  redeemText: { fontSize: 12, fontFamily: typography.fontMedium },
  saveBtn: {
    borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6,
    borderWidth: 0.5, minWidth: 70, alignItems: 'center',
  },
