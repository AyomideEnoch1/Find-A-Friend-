...
const VENUES = [
  ...
  { name: 'Medical Centre', x: 0.78, y: 0.80, color: '#f87171' },
]
    ...
    <SafeAreaView style={[s.container, { backgroundColor: theme.bg }]}>
      ...
      <View
        ...
        <View style={s.mapGrid}>
          {[...Array(6)].map((_, i) => (
            <View key={`h${i}`} style={[s.gridLine, s.gridLineH, { top: `${(i + 1) * 14}%` }]} />
          ))}
          {[...Array(6)].map((_, i) => (
            <View key={`v${i}`} style={[s.gridLine, s.gridLineV, { left: `${(i + 1) * 14}%` }]} />
          ))}
        </View>
        ...
        {VENUES.map((venue, i) => {
          ...
          return (
            <TouchableOpacity
              ...
              <View style={[
                s.pinLabel,
                selectedVenue?.name === venue.name && { backgroundColor: venue.color }
              ]}>
                <Text style={[
                  s.pinText,
      ...
      {selectedVenue && (
        <View style={[s.venueCard, { borderColor: selectedVenue.color }]}>
          <View style={s.venueCardHeader}>
            <View style={[s.venueCardDot, { backgroundColor: selectedVenue.color }]} />
...
const s = StyleSheet.create({
  ...
  mapContainer: {
    height: 240, marginHorizontal: 16, marginBottom: 12,
  ...
  venuePinSelected: { transform: [{ translateX: -20 }, { translateY: -20 }, { scale: 1.2 }] },
  ...
  venueEmpty: { fontSize: 12, color: 'rgba(240,240,255,0.3)', fontStyle: 'italic' },
  ...
  emptyText: { fontSize: 14, fontWeight: '500', color: 'rgba(240,240,255,0.4)' },
  ...
  eventInfo: { flex: 1 },
  eventTitle: { fontSize: 13, fontWeight: '500', color: '#f0f0ff', marginBottom: 2 },
