...
import {
  getMyStats, getLeaderboard, GAME_META,
  type GameType, type LeaderboardEntry, type UserGameStats,
} from '../lib/games'
...
function StatPill({ label, value, color }: { label: string; value: string | number; color: string }) {
  return (
    <View style={{ flex: 1, alignItems: 'center' }}>
      <Text style={{ fontSize: 22, fontFamily: typography.fontExtraBold, color }}>{value}</Text>
      <Text style={{ fontSize: 11, fontFamily: typography.fontRegular, color: 'rgba(255,255,255,0.38)', marginTop: 2 }}>
        {label}
      </Text>
    </View>
  )
...
export default function GamesScreen() {
  ...
  const statFor = (gt: GameType) =>
    stats.find(s => s.game_type === gt) ?? { wins: 0, losses: 0, games_played: 0, game_type: gt }
      ...
      <ScrollView
        ...
        <View style={s.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[s.subtitle, { color: theme.textFaint }]}>Challenge your campus</Text>
          </View>
          <TouchableOpacity
        ...
        {/* Stats banner */}
        <View style={[s.banner, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <StatPill label="Total Wins"   value={totalWins}          color="#fbbf24" />
          <View style={[s.sep, { backgroundColor: theme.border }]} />
          <StatPill label="Games Played" value={totalPlayed}         color={theme.accent} />
          <View style={[s.sep, { backgroundColor: theme.border }]} />
          <StatPill label="Win Rate"     value={`${winRate}%`}       color="#34d399" />
        </View>

        {/* Game cards */}
        <Text style={[s.sectionLabel, { color: theme.text }]}>Choose a Game</Text>
        ...
        {loading ? (
          ...
          GAME_TYPES.map(gt => {
            ...
            return (
              <TouchableOpacity
                key={gt}
                style={[s.gameCard, { backgroundColor: theme.card, borderColor: meta.border }]}
                activeOpacity={0.8}
                ...
                <View style={s.gameInfo}>
                  <Text style={[s.gameLabel, { color: theme.text }]}>{meta.label}</Text>
                  <Text style={[s.gameTagline, { color: theme.textMuted }]}>{meta.tagline}</Text>
                  <View style={s.gameRecord}>
                    <Text style={[s.recordWin, { color: meta.color }]}>{stat.wins}W</Text>
                    <Text style={[s.recordSep, { color: theme.textFaint }]}> · </Text>
                    <Text style={[s.recordLoss, { color: theme.textMuted }]}>{stat.losses}L</Text>
                  </View>
                </View>

                {/* Actions */}
                <View style={s.gameActions}>
                  <View style={[s.playBtn, { backgroundColor: meta.bg, borderColor: meta.border }]}>
                    <Text style={[s.playBtnText, { color: meta.color }]}>Play</Text>
                  </View>
                  <TouchableOpacity
                    onPress={() => router.push(`/leaderboard/${gt}` as any)}
                    style={s.boardLink}>
                    <Text style={[s.boardLinkText, { color: theme.textFaint }]}>Board</Text>
                    <Ionicons name="chevron-forward" size={10} color={theme.textFaint} />
                  </TouchableOpacity>
                </View>
              </TouchableOpacity>
            )
        ...
        <View style={s.lbHeaderRow}>
          <Text style={[s.sectionLabel, { color: theme.text, marginBottom: 0 }]}>🏆 Leaderboard</Text>
          <TouchableOpacity onPress={() => router.push(`/leaderboard/${activeGame}` as any)}>
          ...
          {GAME_TYPES.map(gt => (
            <TouchableOpacity
              ...
              style={[
                s.tab,
                {
                  backgroundColor: activeGame === gt ? theme.accent : theme.card,
                  borderColor:     activeGame === gt ? theme.accent : theme.border,
                },
              ]}>
              <Text style={[s.tabText, { color: activeGame === gt ? '#fff' : theme.textMuted }]}>
            ...
            {leaders.map((entry, i) => (
              <View key={entry.user_id}>
                <View style={s.lbRow}>
                  ...
                  <Avatar url={entry.avatar_url} name={entry.full_name} size={36} theme={theme} />
                  <View style={s.lbMeta}>
                    <Text style={[s.lbName, { color: theme.text }]} numberOfLines={1}>
...
const s = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 16, fontFamily: typography.fontSemiBold, marginBottom: 12 },

  gameCard: {
    flexDirection: 'row', alignItems: 'center', gap: 12,
    borderRadius: 20, padding: 16, marginBottom: 12, borderWidth: 1,
  },
  gameIcon: { width: 58, height: 58, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  gameEmoji: { fontSize: 30 },
  gameInfo: { flex: 1 },
  gameLabel: { fontSize: 15, fontFamily: typography.fontSemiBold },
  gameTagline: { fontSize: 11, fontFamily: typography.fontRegular, marginTop: 2 },
  ...
  lbCard: { borderRadius: 18, borderWidth: StyleSheet.hairlineWidth, overflow: 'hidden' },
  lbRow: {
    flexDirection: 'row', alignItems: 'center',
  ...
  winBadgeText: { fontSize: 12, fontFamily: typography.fontSemiBold, color: '#fbbf24' },
