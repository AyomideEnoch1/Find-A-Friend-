...
import { getEventDetail, updateEvent, uploadEventCover } from '../../lib/events'
...
export default function EditEventScreen() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setCoverUri(data.cover_image_url ?? null)
      
      if (data.starts_at) {
        setStartsAtText(toInputString(new Date(data.starts_at)))
      }
      if (data.ends_at) {
        setEndsAtText(toInputString(new Date(data.ends_at)))
      }
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      let coverImageUrl = coverUri ?? undefined
  ...
  if (loading) {
    return (
      <SafeAreaView style={[s.root, { backgroundColor: theme.bg, alignItems: 'center', justifyContent: 'center' }]}>
